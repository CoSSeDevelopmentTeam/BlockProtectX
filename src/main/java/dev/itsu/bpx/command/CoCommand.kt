...
class CoCommand : Command("co", "ブロックログコマンド。/coを実行後、対象のブロックをタップ", "/co") {
    ...
    override fun execute(sender: CommandSender, s: String, args: Array<String>): Boolean {
        ...
        if (!sender.isOp) {
            sender.sendMessage("§aシステム§r>>このコマンドの実行にはオペレーター権限が必要です。")
            return false
        }
    ...
    companion object {
        ...
        fun process(e: PlayerInteractEvent) {
            ...
            ${TextFormat.RESET}----  ${TextFormat.DARK_AQUA}BlockProtectX [${data.index}/${data.data.size}]  ${TextFormat.RESET}----
            ${TextFormat.RESET}別のブロックをタップすることでリセットできます。
        """.trimIndent())

            e.player.sendMessage("""
            §aステータス ${logToDisplay.status.text}
            §aユーザー[IP] §r${logToDisplay.owner}[${logToDisplay.ip}]
            §aDeviceID §r${logToDisplay.clientId}
            §a時間 §r${SimpleDateFormat("yyyy/MM/dd HH:mm:ss").format(logToDisplay.time)}
            §aブロック (id:damage) §r${logToDisplay.blockId}:${logToDisplay.blockDamage}
            §a座標 §r(${logToDisplay.x}, ${logToDisplay.y}, ${logToDisplay.z}, ${logToDisplay.levelName})
